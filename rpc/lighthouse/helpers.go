...
// Lighthouse may return this when data is pruned, not yet available, or the node is syncing.
...
//   - errUnavailable (HTTP 503), often returned when a Lighthouse node has pruned
...
// Lighthouse APIs without treating them as hard failures.
...
// Some older Lighthouse nodes have been observed returning bogus int64-overflow values.
...
func sanitizeDepositCount(count uint64) uint64 {
	if count > maxInt32 {
		return 0 // ignore bogus value from some older Lighthouse nodes
	}
	return count
}
...
//
// Parameters:
//   - lc: LighthouseClient instance (used for fetching epoch assignments).
