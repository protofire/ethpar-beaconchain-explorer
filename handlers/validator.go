...
func Validator(bt *db.Bigtable) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		lastAttestationSlots, err := bt.GetLastAttestationSlots([]uint64{index})
		if err != nil {
		...
		g.Go(func() error {
			...
			executionIncomeHistoryData, err := getExecutionChartData([]uint64{index}, currency, lowerBoundDay, bt)
			if err != nil {
		...
		g.Go(func() error {
			...
			earnings, balances, err := GetValidatorEarnings([]uint64{index}, currency, bt)
			if err != nil {
		...
		g.Go(func() error {
			...
			if validatorPageData.ExitEpoch != 9223372036854775807 && validatorPageData.ExitEpoch <= validatorPageData.Epoch {
				...
				attestationData, err := bt.GetValidatorAttestationHistory([]uint64{index}, validatorPageData.Epoch, validatorPageData.Epoch)
				if err != nil {
			...
			if validatorPageData.AttestationsCount > 0 {
				...
				if validatorPageData.Epoch > nextStatsDayFirstEpoch {
					...
					missedAttestations, err := bt.GetValidatorMissedAttestationHistory([]uint64{index}, validatorPageData.Epoch-lookback, validatorPageData.Epoch-1)
					if err != nil {
		...
		g.Go(func() error {
			eff, err := bt.GetValidatorEffectiveness([]uint64{index}, validatorPageData.Epoch-1)
			if err != nil {
		...
		g.Go(func() error {
			...
			if len(actualSyncPeriods) > 0 {
				...
				if lastSyncPeriod.LastEpoch > lastExportedEpoch {
					res, err := bt.GetValidatorSyncDutiesHistory([]uint64{index}, (lastExportedEpoch+1)*utils.Config.Chain.ClConfig.SlotsPerEpoch, latestProposedSlot)
					if err != nil {
...
func ValidatorAttestationInclusionEffectiveness(bt *db.Bigtable) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		w.Header().Set("Content-Type", "application/json")
		...
		eff, err := bt.GetValidatorEffectiveness([]uint64{index}, epoch)
		if err != nil {
...
func ValidatorAttestations(bt *db.Bigtable) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		w.Header().Set("Content-Type", "application/json")
		...
		if totalCount > 0 {
			...
			attestationData, err := bt.GetValidatorAttestationHistory([]uint64{index}, uint64(startEpoch), uint64(endEpoch))
			if err != nil {
...
func ValidatorSlashings(w http.ResponseWriter, r *http.Request) {
	...
	for _, b := range attesterSlashings {
		...
		inter := intersect.Simple(b.Attestation1Indices, b.Attestation2Indices)
...
func ValidatorHistory(bt *db.Bigtable) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		w.Header().Set("Content-Type", "application/json")
		currency := GetCurrency(r)
		pageLength := 10
		...
		if postExitEpochs > 0 {
			...
			withdrawalMap, incomeDetails, err := getWithdrawalAndIncome(index, startEpoch, endEpoch, bt)
			if err != nil {
				return
			}
...
func getWithdrawalAndIncome(index uint64, startEpoch uint64, endEpoch uint64, bt *db.Bigtable) (map[uint64]*types.ValidatorWithdrawal, map[uint64]map[uint64]*itypes.ValidatorEpochIncome, error) {
	...
	g.Go(func() error {
		var err error
		incomeDetails, err = bt.GetValidatorIncomeDetailsHistory([]uint64{index}, startEpoch, endEpoch)
		if err != nil {
...
func ValidatorSync(bt *db.Bigtable) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		w.Header().Set("Content-Type", "application/json")
		...
		for _, slotRange := range consecutiveSlotRanges {
			sdh, err := bt.GetValidatorSyncDutiesHistory([]uint64{validatorIndex}, slotRange.StartSlot, slotRange.EndSlot)
			if err != nil {
