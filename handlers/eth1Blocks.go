...
func Eth1BlocksData(bt *db.Bigtable) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if err != nil {
			log.Warnf("error converting datatables draw parameter from string to int for route %v: %v", r.URL.String(), err)
			http.Error(w, "Error: Missing or invalid parameter draw", http.StatusBadRequest)
			return
		}
		start, err := strconv.ParseUint(q.Get("start"), 10, 64)
		if err != nil {
			log.Warnf("error converting datatables start parameter from string to int for route %v: %v", r.URL.String(), err)
			http.Error(w, "Error: Missing or invalid parameter start", http.StatusBadRequest)
			return
		}
		length, err := strconv.ParseUint(q.Get("length"), 10, 64)
		if err != nil {
			log.Warnf("error converting datatables length parameter from string to int for route %v: %v", r.URL.String(), err)
